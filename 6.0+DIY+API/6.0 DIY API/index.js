...
app.get("/jokes/:id", (req,res) => {
  ...
  const specificId= jokes.find( (joke) => joke.id===id );
  res.json(specificId);
});
...
app.get("/filter", (req,res) => {
  ...
  const typeArray=jokes.filter( (joke) => joke.jokeType===type );
  res.json(typeArray);
});
//4. POST a new joke
...
app.put("/jokes/:id", (req,res) => {
  ...
  const current = jokes.findIndex( (joke) => joke.id === id);
  jokes[current] = changed;
  res.json(changed);
});
...
app.delete("/jokes/:id", (req,res) => {
  ...
  if (index > -1){
    ...
    res
      .status(404)
      .json({error :`Joke with id: ${id}, is not found, no jokes deleted` });
  }
});
...
app.delete("/all", (req,res) => {
  ...
  if(userKey===masterKey){
    ...
    res
      .status(404)
      .json({error: `You are not authorised to perform this function. `});
  }
});
